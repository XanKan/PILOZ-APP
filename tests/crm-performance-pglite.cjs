...
(async()=>{
  ...
  try{
    ...
    const activitiesV4=await rpc(db,"select public.get_activity_workspace_v4('list','all',null,null,null,null,null,current_date-30,current_date+60,false,1,200,'activity_at','asc') value");
    ...
    assert(activitiesV4.rows.length<=200&&Number(activitiesV4.total)>0&&Number(activitiesV4.counts.today)>=0,'pagination ou agrégation activités v4 invalide');
