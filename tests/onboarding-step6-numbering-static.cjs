...
const checks={
  ...
  settings_prefixes_are_readonly:settings.includes("field('Préfixe',`${family}_prefix`,prefix,'text','readonly aria-readonly=\"true\"')"),
  numbering_families_are_side_by_side:settings.includes('class="sales-numbering-grid"')&&settings.includes("sequenceCard('quote','Devis'")&&settings.includes("sequenceCard('invoice','Factures'"),
  default_margin_is_in_onboarding:onboarding.includes("input('Marge par défaut (%)','documents.defaultMarginRate'")&&onboarding.includes('default_margin_rate'),
  default_margin_is_in_sales_settings:settings.includes("field('Marge par défaut (%)','default_margin_rate'")&&settings.includes('sales_settings:{...(erp.sales_settings||{}),default_margin_rate:margin}'),
