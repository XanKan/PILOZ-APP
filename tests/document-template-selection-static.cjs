...
const checks = [
  ...
  ['save and quote validation prepare the configured preview before consultation', editor.includes('await global.PilozDocumentViewerV2.primeDocumentPreview(savedId)') && !viewer.includes("context.frozen?'Le contenu figé reste consultable.")],
