...
const checks = [
  ...
  ['quote conversion proposes one invoice draft then progress mode in editor', viewer.includes("PilozDocumentViewerV2.convert('invoice')") && !viewer.includes("PilozDocumentViewerV2.convert('progress_invoice')") && editor.includes('Facture de situation') && editor.includes('toggleProgressMode')],
