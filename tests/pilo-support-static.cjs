...
assert.ok(app.includes("direct.closest?.('details')") && app.includes("disclosure.open=true"), "Le menu Convertir en doit rester ouvert lorsque la formation encadre le bouton Facture");
...
assert.ok(app.includes("trainingAction:'open-payment'") && app.includes("requireSelector:'#due-payment-form'") && app.includes("PilozCommercialWorkspace?.openDuePayment?.(TRAINING_PAYMENT_INVOICE_ID)") && app.includes("if(result){completeTrainingAction(true);return;}"), "Le clic Saisir un règlement doit ouvrir la vraie fenêtre avant de guider le montant");
