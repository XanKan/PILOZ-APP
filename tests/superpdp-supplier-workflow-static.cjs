...
const checks={
  ...
  sensitive_reason_required:edge.includes('BUYER_LIFECYCLE_REASONS[statusCode]')&&edge.includes('superpdp_lifecycle_reason_required')&&edge.includes('superpdp_lifecycle_note_required'),
  afnor_reason_validated:edge.includes('allowedReasons.has(reasonCode)')&&edge.includes('superpdp_lifecycle_reason_invalid'),
  afnor_reason_sent:edge.includes('detail.reason = reasonCode')&&edge.includes('requestPayload.details = [detail]'),
  ...
  reason_modal:supplier.includes('Type de motif *')&&supplier.includes('name="reasonCode" required')&&supplier.includes('Explication transmise au fournisseur *')&&supplier.includes('required maxlength="1200"'),
  status_specific_reasons:['AUTRE','TX_TVA_ERR','JUSTIF_ABS','CMD_ERR','PU_ERR','LIVR_INCOMP'].every(code=>supplier.includes(code)),
