...
Deno.serve(async(req)=>{
 ...
 const {data,error}=await admin.rpc('save_document_template_version',{target_company_id:body.companyId,target_user_id:user.id,target_template_id:body.templateId||null,target_name:String(body.name||'').trim(),target_document_type:body.documentType,target_language:body.language||'fr',target_status:body.status||'active',target_is_default:!!body.isDefault,target_visual_schema:body.visualSchema||{},target_html:cleanHtml(body.html),target_css:sanitizeCss(body.css),target_comment:body.comment||'Nouvelle version'});
