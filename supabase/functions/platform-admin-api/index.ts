 ...
 try{
  ...
  if(action==="demo_accounts.create"){
   ...
   try{
    ...
    const {error:documentSettingsError}=await service.from("company_document_settings").upsert({company_id:createdCompanyId,quote_prefix:"DEV-DEMO",invoice_prefix:"FAC-DEMO"},{onConflict:"company_id",ignoreDuplicates:true});
    if(documentSettingsError)throw documentSettingsError;
    const {error:settingsError}=await service.from("company_settings").update({legal_name:"Horizon Conseil (démonstration)",trade_name:"Horizon Conseil",siren:"123456789",siret:"12345678900012",address_line1:"18 avenue des Ateliers",postal_code:"85000",city:"La Roche-sur-Yon",country:"France",country_code:"FR",email:"contact@horizon-conseil.example",phone_e164:"+33100000000",currency:"EUR",language:"fr",subject_to_vat:true,default_vat_rate:20,onboarding_step:7,onboarding_completed_at:now.toISOString()}).eq("company_id",createdCompanyId);
