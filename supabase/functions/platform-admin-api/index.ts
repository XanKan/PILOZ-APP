 ...
 try{
  ...
  if(action==="companies.detail"){
   ...
   const [company,settings,subscription,members,features,events,support,billing,compliance,anomalies,privacy]=await Promise.all([
    ...
    permissions.has("compliance.read")?client.from("company_fiscal_configurations").select("mode,activation_status,application_version,schema_version,signing_status,last_integrity_check_at,updated_at").eq("company_id",companyId).maybeSingle():Promise.resolve({data:null,error:null}),
    permissions.has("compliance.read")?client.from("compliance_anomalies").select("id,anomaly_type,severity,source,detected_at,resolved_at").eq("company_id",companyId).order("detected_at",{ascending:false}).limit(50):Promise.resolve({data:[],error:null}),
