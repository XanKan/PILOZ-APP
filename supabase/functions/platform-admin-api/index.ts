...
Deno.serve(async req=>{
 ...
 const audit=async(actionName:string,targetType:string,targetId:string|null,companyId:string|null,oldState:unknown,newState:unknown,reason?:unknown)=>{
  const {error}=await privileged().rpc("append_platform_admin_audit_service",{target_actor_user_id:context.user_id,target_action:actionName,target_type:targetType,target_id:targetId,target_company_id:companyId,old_state:oldState??null,new_state:newState??null,target_reason:text(reason,500)||null});
 ...
 try{
  ...
  if(action==="demo_accounts.create"){
   ...
   try{
    ...
    const {data:company,error:companyError}=await client.rpc("platform_admin_create_company",{target_owner_user_id:createdUserId,target_company:{owner_first_name:firstName,owner_last_name:lastName,owner_email:ownerEmail,provisioning_name:demoName,provisioning_pending:false,legal_name:"Horizon Conseil (démonstration)",trade_name:"Horizon Conseil",company_type:"company",legal_form:"Société de démonstration",siren:"123456789",siret:"12345678900012",address_line1:"18 avenue des Ateliers",postal_code:"85000",city:"La Roche-sur-Yon",country:"France",country_code:"FR",email:"contact@horizon-conseil.example",phone:"+33100000000",currency:"EUR",language:"fr",admin_tags:["demo","seeded"],internal_admin_notes:"Compte de démonstration Piloz — données entièrement fictives."},target_subscription:{plan_version_id:plan.id,billing_interval:"monthly",status:"trialing",trial_days:14,max_users:null},target_reason:reason});
    if(companyError)throw companyError;createdCompanyId=company.id;
    const {data:createdCompany,error:createdCompanyError}=await service.from("companies").select("id,admin_tags").eq("id",createdCompanyId).maybeSingle();
    if(createdCompanyError||!createdCompany)throw createdCompanyError||new Error("L'entreprise de démonstration créée est introuvable.");
    const demoTags=Array.from(new Set([...(Array.isArray(createdCompany.admin_tags)?createdCompany.admin_tags:[]),"demo","seeded"]));
    const {error:demoTagError}=await service.from("companies").update({admin_tags:demoTags}).eq("id",createdCompanyId);
    if(demoTagError)throw demoTagError;
    const now=new Date(),tomorrow=new Date(now.getTime()+86_400_000),nextWeek=new Date(now.getTime()+7*86_400_000);
    const {error:documentSettingsError}=await service.from("company_document_settings").upsert({company_id:createdCompanyId,quote_prefix:"DEV-DEMO",invoice_prefix:"FAC-DEMO"},{onConflict:"company_id"});
    if(documentSettingsError)throw documentSettingsError;
    const {data:completedSettings,error:settingsError}=await service.from("company_settings").update({legal_name:"Horizon Conseil (démonstration)",trade_name:"Horizon Conseil",siren:"123456789",siret:"12345678900012",address_line1:"18 avenue des Ateliers",postal_code:"85000",city:"La Roche-sur-Yon",country:"France",country_code:"FR",email:"contact@horizon-conseil.example",phone_e164:"+33100000000",currency:"EUR",language:"fr",subject_to_vat:true,default_vat_rate:20,onboarding_step:7,onboarding_completed_at:now.toISOString()}).eq("company_id",createdCompanyId).select("company_id,onboarding_step,onboarding_completed_at").maybeSingle();
    if(settingsError||!completedSettings)throw settingsError||new Error("La configuration de démonstration n'a pas pu être finalisée.");
    const {data:completedPreferences,error:preferencesError}=await service.from("user_preferences").upsert({user_id:createdUserId,company_id:createdCompanyId,onboarding_completed:true},{onConflict:"user_id"}).select("user_id,onboarding_completed").maybeSingle();
    if(preferencesError||!completedPreferences?.onboarding_completed)throw preferencesError||new Error("L'onboarding du compte de démonstration n'a pas pu être validé.");
