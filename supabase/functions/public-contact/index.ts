...
Deno.serve(async (req) => {
  ...
  if (!resendKey) {
    console.error("[PILOZ Contact] RESEND_API_KEY absente");
    return json(req, { error: "L’envoi est temporairement indisponible." }, 503);
  }
  ...
  if (!providerResponse.ok) {
    console.error("[PILOZ Contact] Échec du fournisseur e-mail", { status: providerResponse.status });
    return json(req, { error: "Le message n’a pas pu être envoyé. Réessayez dans quelques instants." }, 502);
  }
