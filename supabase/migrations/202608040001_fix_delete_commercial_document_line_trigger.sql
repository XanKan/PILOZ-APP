...
-- delete_commercial_document deleted the parent documents row and relied on
-- ON DELETE CASCADE to remove document_lines. That cascade delete still fires
-- enforce_document_line_permission() per line, which looks up the parent
-- document by id — but by the time the cascade runs, the parent row is
-- already gone, so the trigger always raised 'document_not_found' (23503)
-- and every deletion of a draft quote/invoice failed.
-- Fix: delete document_lines explicitly before the parent row, so the
-- permission trigger still finds the (still-locked-open-for-update) parent.
...
begin
  ...
  delete from public.document_lines where document_id=doc.id;
