(function(global){
 ...
 const documentNumber=doc=>doc?.number||doc?.draft_number||'Brouillon';
 ...
 function ensureActive(data){
  ...
  if(!doc){const requestedKind=params.get('type');if(['quote','invoice'].includes(requestedKind))ui.kind=requestedKind;doc=documentsFor(data)[0]||null;ui.id=doc?.id||'';}
  return doc;
 }
 ...
 function renderPreview(data,doc){
  ...
  return`<main class="document-viewer-preview" aria-label="Aperçu du document"><header class="document-viewer-preview-head"><div>${iconButton('Replier ou afficher la liste','☰','PilozDocumentViewerV2.toggleList()')}<button type="button" class="document-viewer-mobile-back" onclick="PilozDocumentViewerV2.backToList()">← Liste</button><b>${esc(documentNumber(doc))}</b><span>${esc(status)}</span></div><div>${actionButton('Informations','PilozDocumentViewerV2.toggleInfo()','secondary')}</div></header>
   ...
   <div class="document-viewer-stage" id="document-viewer-stage">${pdfUrl?`<iframe title="PDF final ${esc(documentNumber(doc))}" src="${esc(pdfUrl)}#page=${ui.page}&zoom=${ui.zoom}&toolbar=0&navpanes=0"></iframe>`:`<div class="document-viewer-fallback-notice ${context.frozen?'frozen':''}"><b>${context.frozen?'Le contenu figé reste consultable.':'Aperçu non définitif'}</b><span>${context.frozen?'Le PDF final sera utilisé automatiquement dès qu’il sera disponible.':'Finalisez le document pour créer son instantané et son PDF définitif.'}</span></div><div class="document-viewer-zoom-wrap" style="--viewer-scale:${ui.zoom/100}"><div>${renderFallbackPage(context,ui.page)}</div></div>`}</div>
  </main>`;
 }
 ...
 function renderQuoteActions(data,doc){
  const invoiced=quoteInvoiced(data,doc),remaining=Math.max(0,Number(doc.total_incl_tax||0)-invoiced),locked=!!(doc.finalized_at||doc.validated_at||doc.locked_at);
 ...
 function renderViewer(runtimeState){
  ...
  if(!doc){main.innerHTML=renderEmpty(ui.kind);return true;}
 ...
 function select(id){const doc=(state()?.data?.documents||[]).find(row=>row.id===id);if(!doc)return;ui.id=id;ui.kind=documentKind(doc);ui.tab='all';ui.status='all';ui.page=1;ui.zoom=85;ui.mobileDocument=true;updateHash(id);renderViewer(state());}
 ...
 function backToList(){ui.mobileDocument=false;ui.infoOpen=false;renderViewer(state());}
 ...
 function currentPdf(){const data=state().data,doc=activeDocument(data),snapshot=latestSnapshot(data,doc),path=pdfPath(doc,snapshot);return{doc,snapshot,path,url:ui.pdfUrls.get(path)||''};}
 function download(){const current=currentPdf();if(current.url){const link=document.createElement('a');link.href=current.url;link.download=`${documentNumber(current.doc)}.pdf`;link.rel='noopener';document.body.appendChild(link);link.click();link.remove();return;}notify('Le PDF final n’est pas encore disponible. L’aperçu imprimable a été ouvert.','info');print();}
 ...
 async function runConversion(rpcName,params){
  ui.busy=true;try{const id=await api().rpc(rpcName,params);ui.modal=null;await app().refresh();if(id)select(id);notify('Brouillon de facturation créé et lié au devis.','success');return id;}
 ...
 global.PilozDocumentViewerV2={open,close,create,select,search,setTab,setStatus,toggleList,toggleInfo,backToList,previousDocument,nextDocument,changeZoom,changePage,download,print,fullscreen,openPayment,openPaymentCancellation,closeModal,savePayment,cancelPayment,editComment,cancelCommentEdit,saveComment,deleteComment,convert,setDepositMode,saveConversion,openCredit,saveCredit,appAction,archive,showHistory,renderViewer,snapshotContext,filteredDocuments,remainingFor};
})(window);
