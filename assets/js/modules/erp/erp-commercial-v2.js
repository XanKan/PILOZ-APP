(function(global){
 ...
 function areaFor(path){if(['sales/clients','library/prospects','purchases/suppliers','sales/catalog'].some(item=>path===item||path.startsWith(item+'/')))return'library';if(path.startsWith('crm/'))return'crm';if(path.startsWith('sales/'))return'sales';if(path.startsWith('purchases/'))return'purchases';return'';}
 ...
 function renderNavigation(node,s,current){
  ...
  if(body)body.innerHTML=data.items.map(([itemPath,label])=>{const active=secondaryActive(path,itemPath);return`<div class="modern-secondary-entry ${active?'active':''}"><button class="modern-secondary-item ${active?'active':''}" data-route="${itemPath}" onclick="PilozModern.selectSecondary('${itemPath}')" ${active?'aria-current="page"':''}><span>${esc(label)}</span></button><button class="modern-secondary-new-tab" type="button" onclick="event.stopPropagation();PilozModern.openInNewTab('${itemPath}')" title="Ouvrir ${esc(label)} dans un nouvel onglet" aria-label="Ouvrir ${esc(label)} dans un nouvel onglet"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 4h6v6M20 4l-9 9"/><path d="M18 13v6a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V7a1 1 0 0 1 1-1h6"/></svg></button></div>`;}).join('');
 }
 function openArea(area){const target={crm:'crm/pipeline',sales:'sales/quotes',purchases:'purchases/orders',library:'sales/clients',accounting:'accounting/payments',settings:'settings/overview',dashboard:'dashboard',reports:'reports'}[area]||'dashboard';base.openArea(area,target);}

 const defaultBlocks=['revenue','collected','outstanding','overdue','quotes','activities','pipeline','recent'];
 const optionalBlocks=['topClients','topItems','margin','comparison','supplierOrders','receipts'];
 const blockLabels={revenue:'Chiffre d’affaires',collected:'Montant encaissé',outstanding:'Reste à encaisser',overdue:'Factures en retard',quotes:'Devis en attente',activities:'Mes activités',pipeline:'Pipeline commercial',recent:'Activité récente',topClients:'Top clients',topItems:'Top articles',margin:'Marge brute',comparison:'Comparaison N / N-1',supplierOrders:'Commandes fournisseurs en attente',receipts:'Réceptions attendues'};
 ...
 function dashboardData(s){
  const range=periodRange(dashboardUi.period),previousRange=periodRange(dashboardUi.period,true),docs=s.data.documents||[],statusApi=global.PilozDocumentStatus,docStatus=doc=>statusApi?.effective?.(s.data,doc)||doc.status,invoiceTypes=['invoice','deposit_invoice','balance_invoice','credit_note'],valid=doc=>invoiceTypes.includes(doc.document_type)&&!['draft','cancelled','archived'].includes(docStatus(doc)),signed=doc=>doc.document_type==='credit_note'?-1:1,current=docs.filter(doc=>valid(doc)&&inRange(doc.issue_date,range)),previous=docs.filter(doc=>valid(doc)&&inRange(doc.issue_date,previousRange)),revenue=current.reduce((sum,doc)=>sum+signed(doc)*Number(doc.total_excl_tax||0),0),previousRevenue=previous.reduce((sum,doc)=>sum+signed(doc)*Number(doc.total_excl_tax||0),0),cost=current.reduce((sum,doc)=>sum+signed(doc)*Number(doc.total_cost||0),0),payments=(s.data.payments||[]).filter(row=>statusApi?.confirmed?.(row)??row.status==='confirmed'),collected=payments.filter(row=>inRange(row.paid_at,range)).reduce((sum,row)=>sum+Number(row.amount||0),0),invoices=docs.filter(doc=>['invoice','deposit_invoice','balance_invoice'].includes(doc.document_type)&&!['cancelled','archived'].includes(docStatus(doc))),outstandingRows=invoices.map(doc=>({...doc,remaining:statusApi?.remainingFor?.(s.data,doc)??Math.max(0,Number(doc.total_incl_tax||0)-paidFor(s,doc.id))})).filter(doc=>doc.remaining>0),outstanding=outstandingRows.reduce((sum,doc)=>sum+doc.remaining,0),today=iso(new Date()),overdue=outstandingRows.filter(doc=>docStatus(doc)==='overdue'),quotes=docs.filter(doc=>doc.document_type==='quote'&&['draft','pending'].includes(docStatus(doc))),activities=(s.data.activities||[]).filter(row=>{const meta=row.metadata||{},statusValue=row.status||meta.status||(row.completed_at?'completed':'todo'),assignees=meta.assigned_user_ids||[];return!['completed','cancelled'].includes(statusValue)&&(!row.assigned_user_id&&assignees.length===0||row.assigned_user_id===global.PilozRuntime.session.user_id||assignees.includes(global.PilozRuntime.session.user_id));}),quotePipeline=docs.filter(doc=>doc.document_type==='quote'&&!['cancelled','archived'].includes(docStatus(doc))&&!doc.archived_at).map(quote=>global.PilozCommercialWorkspace?.quotePipelineMetrics?.(s,quote)||{quote,stage:quote.metadata?.pipeline_stage||quote.status||'draft'}),pipelineQuotes=quotePipeline.filter(row=>!['rejected','expired','collected'].includes(row.stage)),pipeline=pipelineQuotes.reduce((sum,row)=>sum+Number(row.quote.total_excl_tax||0),0),logs=(s.data.activityLogs||[]).slice(0,8);
  return{range,revenue,previousRevenue,cost,collected,outstanding,outstandingRows,overdue,quotes,activities,pipelineQuotes,pipeline,logs};
 }
 function dashboardKeys(s){
  const context=`${s.companyId||''}:${global.PilozRuntime?.session?.user_id||''}`,aliases={overdueInvoices:'overdue',quotesPending:'quotes',todayReminders:'activities',supplierOrdersPending:'supplierOrders',receiptsExpected:'receipts',comparison:'comparison'};
 ...
 function dashboardSpan(key){return['quotes','activities','pipeline','recent','topClients','topItems'].includes(key)?2:1;}
 ...
 function dashboardBlock(key,s,d){
  ...
  if(key==='quotes')return`<div class="commercial-list">${d.quotes.slice(0,6).map(row=>`<button onclick="PilozModern.openDocumentInViewer('${row.id}')"><span><b>${esc(row.number||'Brouillon')}</b><small>${esc(row.subject||'Sans objet')}</small></span><strong>${money(row.total_incl_tax)}</strong></button>`).join('')||'<p>Aucun devis en attente.</p>'}</div>`;
