(function(global){
 ...
 let view={busy:false,production:null,events:[],disconnectConfirm:false,error:'',success:''};
 ...
 function draw(){
  ...
  main.innerHTML=header()+`${view.error?`<div class="phase1-alert phase1-alert-danger">${esc(view.error)}</div>`:''}${view.success?`<div class="phase1-alert phase1-alert-success">${esc(view.success)}</div>`:''}${productionCard()}${auditCard()}`;
 }
 ...
 async function load(){
  ...
  try{
   ...
   view.production=production||null;view.events=events||[];global.PilozSuperPdpStatus=view.production;
 ...
 global.PilozElectronicInvoicing={startProduction,productionStatus,ensureDirectory,refreshProduction,activateDirectory,askDisconnect,cancelDisconnect,disconnect,reload:load};
})(window);
