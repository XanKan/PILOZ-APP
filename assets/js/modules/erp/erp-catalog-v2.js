(function (global) {
  ...
  function defaultMarginRate() {
    const value = Number(
      state().data.erpSettings?.[0]?.sales_settings?.default_margin_rate,
    );
  ...
  function itemForm(item = {}) {
    const rates = state().data.vatRates || [],
      ...
      purchaseFields = canPurchase
        ? `${field("Prix d’achat HT", "purchase_price", item.purchase_price || 0, "number", 'min="0" step="0.0001" oninput="PilozCatalog.syncPrices(\'cost\')"')}${field("Frais d’approche", "landing_cost", item.landing_cost || 0, "number", 'min="0" step="0.0001" oninput="PilozCatalog.syncPrices(\'cost\')"')}${field("Coût de revient", "cost_price", cost(item), "number", "readonly")}`
      ...
      marginFields = canMargin
        ? field(
            "Marge cible (%)",
            "margin_rate",
            item.id && marginRate(item) !== null
              ? marginRate(item).toFixed(2)
              : defaultMarginRate().toFixed(2),
            "number",
  ...
  function formPayload(form, existing = {}) {
    ...
    raw.stock_managed = !!existing.stock_managed;
    raw.track_lots = !!existing.track_lots;
    raw.track_serials = !!existing.track_serials;
    raw.recurring = !!existing.recurring;
    for (const key of [
      "sale_price_ttc",
      "margin_rate",
      "primary_supplier_id",
  ...
  async function saveItem(id = "") {
    ...
    const raw = Object.fromEntries(new FormData(form)),
      existingItem = id ? catalog().find((x) => x.id === id) || {} : {},
      payload = formPayload(form, existingItem),
  ...
  function variantsTab(item) {
    ...
    return `<section class="catalog-panel"><div class="catalog-panel-head"><div><h2>Variantes</h2><p>Références, codes-barres et prix propres à chaque combinaison.</p></div><div class="catalog-top-actions"><button class="btn btn-o" onclick="PilozCatalog.openVariantMatrix('${item.id}')">Générer une matrice</button><button class="btn btn-p" onclick="PilozCatalog.openVariant('${item.id}')">Ajouter une variante</button></div></div>${
      rows.length
        ...
        : '<div class="catalog-empty"><b>Article simple</b><p>Ajoutez des variantes uniquement si les déclinaisons ont leurs propres références ou prix.</p></div>'
    }</section>`;
  }
