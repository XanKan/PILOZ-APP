(function(global){
  ...
  function renderChart(payload){
    ...
    return`<section class="cockpit-chart-panel"><header><div><span>Analyse</span><h2>Performance de l’activité</h2><p>${esc(periodLabel(summary))}</p></div>${chartControls(summary)}</header><div class="cockpit-chart-legend">${series.map(([key,label],index)=>`<span><i style="--legend:${colors[index]}"></i>${esc(label)}</span>`).join('')}</div><div class="cockpit-chart" role="img" aria-label="Évolution de la performance sur la période"><svg viewBox="-8 -8 ${width+16} ${height+36}" preserveAspectRatio="none"><g class="cockpit-chart-grid">${grid}</g>${paths}${dots}${labels.map(index=>`<text x="${(index/(Math.max(1,points.length-1))*width).toFixed(1)}" y="${height+24}" text-anchor="${index===0?'start':index===points.length-1?'end':'middle'}">${esc(date(points[index]?.date))}</text>`).join('')}</svg></div><table class="sr-only"><caption>Données du graphique de performance</caption><thead><tr><th>Date</th>${series.map(([,label])=>`<th>${esc(label)}</th>`).join('')}</tr></thead><tbody>${accessible}</tbody></table>${renderPerformanceSummary(summary)}</section>`;
  }
