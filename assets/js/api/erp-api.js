(function(global){
  async function request(path,options={}){
    ...
    if(!response.ok){let message='Une erreur est survenue.';try{const data=await response.json();message=data.error||data.message||message;}catch{}throw new Error(message);}
  ...
  async function invoke(name,body,signal){
    ...
    const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'Service indisponible.');return data;
  }
  ...
  global.PilozERP={request,companyContext,invoke,upsertCompanySettings,query,insert,update,remove,rpc,upload,signedUrl};
})(window);
